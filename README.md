...
使用记事本或者其他文本编辑器，打开 `config.js` 的文件即可进行一些参数的修改，下面列举一些重要的参数：

- `color`，格式为 `JSON` 对象，`name` 的值与其对应的颜色，用来设置柱状图的填充颜色
- `itemLabel`，左上角的固定文字
- `typeLabel`，右上角的固定文字
- `divide_by`，右上角的动态文字，取值为数据源的某一列（不能是 `name`），不能与 `use_counter` 同时设置
- `use_counter`，右上角的动态文字，计数上榜次数，不能与 `divide_by` 同时设置
- `showLabel`，开启 `y` 轴标签，取值为 `name` 列
- `use_img`，是否使用图片，不使用会自动关闭左侧圆圈显示，使用需要配置图片列表（否则只显示空心圆圈）
- `imgs`，图片列表，需要配合 `use_img` 使用
- `rounded_rectangle`，是否设置柱状图为圆角
- `labelx`，`label x` 轴位置，设置为负数，表示以柱状图左侧为起点的坐标，偏左（如果使用图片 `use_img`，则此值需要设置大一点，避免文字、图片交叉覆盖）
- `use_divide_by`，右上角的动态文字开启为 `divide_by` 指定列，例如：`value`（官方目前默认为 `type`）
